...
public class AlternatingTreeTest extends TestCase {
  ...
  protected final void testPredict(AlternatingTree t1, AlternatingTree t2) {
    for (int i = 0; i < m_trainLabels.length; i++) {
      ...
      System.out.flush();
      double EPS = 0.000001;
      assertEquals(t1.predict(test).getClassScores()[0], t1.orderPredict(test, m_numRounds - 1).getClassScores()[0], EPS);
      assertEquals(t1.predict(test).getClassScores()[1], t1.orderPredict(test, m_numRounds - 1).getClassScores()[1], EPS);

      assertEquals(t2.predict(test).getClassScores()[0], t2.orderPredict(test, m_numRounds - 1).getClassScores()[0], EPS);
      assertEquals(t2.predict(test).getClassScores()[1], t2.orderPredict(test, m_numRounds - 1).getClassScores()[1], EPS);
    }
