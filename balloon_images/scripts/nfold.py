...
def learner(atreeoption, k, rounds, booster):
    ...
    ###stem = os.path.splitext(datafile)[0]
    ###jboost_output = '-m %s_scoring_function.m -c %s_scoring_function.c'%(stem,stem)
    command = 'java -Xmx1500M -cp "%s"'%os.getenv('CLASSPATH') \
	      + ' jboost.controller.Controller -b ' + booster + ' -p 3 -a -1 -S trial' + str(k) \
	      + ' -n trial.spec -ATreeType ' + atreeoption + ' -numRounds ' + str(rounds) ###+ jboost_output
    if (config != None):
